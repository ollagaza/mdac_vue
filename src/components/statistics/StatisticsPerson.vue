...
export default {
  ...
  mounted() {
    ...
    apistatistics.getProjectInfo(data)
      .then((result) => {
  ...
  methods: {
    ...
    fnClassList(pg) {
      ...
      apistatistics.getClassInfo(data) // 클래스 API 호출
        .then((result) => {
    ...
    class_used_change(sendParam, setDate) {
      ...
      apistatistics.setClassUsed(params).then((data) => {
    ...
    class_delete(sendParam, setDate) {
      ...
      params.used = sendParam.itype;
      params.classes = arrData;
      apistatistics.delClass(params).then((data) => {
        console.log(`data.error===${data.error}`)
        if (data.error === 0) {
          EventBus.emit('alertPopupOpen', null, sendParam.close_msg, null);
          this.fnClassList(1);
