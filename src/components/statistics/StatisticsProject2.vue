  ...
  <div class="layout">
    <div class="layout2" style="width: 100%;">
      <div style="display:flex; flex-direction: row;" >
        <Statistics_Left v-bind:menu_id="22"></Statistics_Left>
        <div style="flex: 2; padding-top: 14px;">
          ...
          <div class="searchWrap">
            <div style="display: flex; flex-direction: row; justify-content: center;">
              ...
              <select class="text" v-model="search_type" style="width: 220px;height: 36px;" @change="fnStatisticsList()">
                <option value="NOW" selected=true>조회기준(기간)</option>
                <option value="SUM" selected=true>조회기준(전체:반려제외)</option>
                <option value="SUMC" selected=true>조회기준(전체:반려포함)</option>
              </select>
          ...
          <div class=" ">
            <div style="height: fit-content;display: flex; flex-direction: row;">
              <div class="btn" v-bind:class="{ deepgreen: search_seq === '33' }" style="margin-left:5px;width:80px; height: 36px;" v-on:click="statisticsGo('33')">통합검수</div>
              <div class="btn" v-bind:class="{ deepgreen: search_seq === '44' }" style="margin-left:5px;width:80px; height: 36px;" v-on:click="statisticsGo('44')">별도검수</div>
          ...
          <div style="padding: 10px 0 0 0 ;">
            <div v-if="search_seq === '33' && search_type === 'NOW'" class="grid_m class_check3 header">
              <div>프로젝트</div>
              <div>전체할당량</div>
              <div>기간할당량</div>
              <div>라벨링진행</div>
            ...
            <div v-if="search_seq === '33' && search_type !== 'NOW'" class="grid_m class_check3_all header">
              <div>프로젝트</div>
              <div>전체할당량</div>
              <div>라벨링진행</div>
              <div>라벨링완료</div>
            ...
            <div style="font-size: 10px" v-if="search_seq === '44' && search_type === 'NOW'" class="grid_m class_check4 header">
              <div>프로젝트</div>
              <div>전체할당량</div>
              <div>기간할당량</div>
              <div>라벨링진행</div>
            ...
            <div style="font-size: 10px" v-if="search_seq === '44' && search_type !== 'NOW'" class="grid_m class_check4_all header">
              <div>프로젝트</div>
              <div>전체할당량</div>
              <div>라벨링진행</div>
              <div>라벨링완료</div>
            ...
            <template v-if="statistics_list.length === 0">
              <div class="grid_m nodata" v-bind:class="{class_check3: search_seq === '33', class_check4: search_seq === '44'}">
            ...
            <template v-if="statistics_list.length > 0">
              <template v-for="(pStatistics, index) in statistics_list">
                <div class="grid_m class_check3 body" v-if="search_seq === '33' && search_type === 'NOW'" v-on:click="one_chart(pStatistics)">
                ...
                <div class="grid_m class_check3_all body" v-if="search_seq === '33' && search_type !== 'NOW'" v-on:click="one_chart(pStatistics)">
                  <div>{{ pStatistics.project_name }}</div>
                  <div>{{ pStatistics.label_total }}</div>
                  <div>{{ pStatistics.label_ing }}</div>
                  <div>{{ pStatistics.label_complete }}</div>
                ...
                <div class="grid_m class_check4 body" v-if="search_seq === '44' && search_type === 'NOW'" v-on:click="one_chart(pStatistics)">
                  <div>{{ pStatistics.project_name }}</div>
                  <div>{{ pStatistics.label_total_all }}</div>
                  <div>{{ pStatistics.label_total }}</div>
                  <div>{{ pStatistics.label_ing }}</div>
                ...
                <div class="grid_m class_check4_all body" v-if="search_seq === '44' && search_type !== 'NOW'" v-on:click="one_chart(pStatistics)">
                  <div>{{ pStatistics.project_name }}</div>
                  <div>{{ pStatistics.label_total_all }}</div>
                  <div>{{ pStatistics.label_ing }}</div>
                  <div>{{ pStatistics.label_complete }}</div>
...
export default {
  ...
  data() {
    return {
      ...
      search_seq: this.$route.params.search_seq ? this.$route.params.search_seq: '33',            // 조회종류(1:라벨러/2:검수자/3:프로젝트)
  ...
  watch: {
    '$route': function(){
      this.search_seq = this.$route.params.search_seq ? this.$route.params.search_seq: '33';
  ...
  methods: {
    ...
    fnStatisticsList() {
      ...
      this.search_seq = String(this.$route.params.search_seq) ? String(this.$route.params.search_seq): '33';
    ...
    statisticsGo(search_seq) {
      ...
      this.$router.push({ name: 'statisticsproject2', params: { search_seq: search_seq }});
    },
    // Data 툴팁 보이기/안보이기
    viewTooltips() {
